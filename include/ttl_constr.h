...
//         Gunnar Staff  <gunnar.staff@sintef.no>
//                                                                           
// Revision: $Id: ttl_constr.h,v 1.8 2004-01-21 12:23:08 bsp Exp $
//                                                                           
// Description:
//                                                                           
//===========================================================================
// Copyright (c) 2001 SINTEF Applied Mathematics
//===========================================================================
...
// INSERT A CONSTRAINED/PRESPECIFIED EDGE BETWEEN TWO EXISTING NODES
// ---------------------------------------------------------------------------- 
namespace ttl_constr {
  
  //  ??? A constant used to avluate a numerical expression against a user spesified
  ...
  // ----------------------------------------------------------------------------------
  /**
  * Check if \e dart has start and end points in \e dstart and \e dend
  *
  * \param dart The dart that should be controlled to see if it's the constraint
  * \param dstart A CCW dart with the startnode of the constraint as the startnode
  * \param dend A CCW dart with the endnode of the constraint as the startnode
  *
  * \retval bool A bool confirming that it's the constraint or not 
  *
  * \using
  *   ttl::same_0_orbit
  */
  ...
  // ----------------------------------------------------------------------------------
  /**
  * Cheks if \e d1 and \e d2 are on the same side of the line between \e dstart and \e dend.
  * (The start nodes of \e d1 and \e d2 represent an edge).
  *
  * \param dstart A CCW dart with the start node of the constraint as the source node of the dart.
  * \param dend A CCW dart with the end node of the constraint as the source node of the dart.
  * \param d1 A CCW dart with the first node as the start node of the dart.
  * \param d2 A CCW dart with the other node as the start node of the dart.
  *
  * \using
  *   TraitsType::orient2d
  */
  ...
  * at the 0-orbit of dstart. <BR>
  * <BR>
  * Assumes: <BR>
  * - CCW dstart and dend, but returned dart can be CW at the boundary. <BR>
  * - Boundary is convex? <BR>
  *
  * \param dstart A CCW dart dstart
  * \param dend A CCW dart dend
  *
  * \retval DartType The dart \e d making the smallest positive (or == 0) angle
  *
  * \using
  *   ttl::isBoundaryNode
  ...
  */
  template <class TraitsType, class DartType>
    ...
    double o_iter = TraitsType::orient2d(d_iter, d0, dend);
    if (o_iter == 0) { // collinnear BUT can be on "back side"
      d0.alpha1().alpha0(); // CW
...
/*     typename TraitsType::real_type o_next; */
  ...
  *
  * Strategy:
  * - Iterate such that d_iter is always strictly "below" the constraint
  *   as seen with dstart to the left and dend to the right.
  ...
  *
  *    Action:
  * 1) Find cone/opening angle containing dstart and dend
  * 2) Find first edge from the first 0-orbit that intersects
  * 3) Check which of the two opposite that intersects
  *
  *   1)  
  *   Rotate CCW and find the (only) case where d_iter and d_next satisfy:
  ...
  * \param dstart A CCW dart with the startnode of the constraint as the startnode
  * \param dend A CCW dart with the endnode of the constraint as the startnode
  * \param elist A list where all the edges crossing the spesified constraint will be put
  * \retval dartType Returns the next "collinear" starting node such that dend is returned
  *                  when done.
  *
  */
  template <class TraitsType, class DartType, class ListType>
    ...
    double orient = TraitsType::orient2d(dstart, d_iter, dend);
  ...
  *   These edges are NOT moved to the end of the list. They are later swapped to none-crossing edges 
  *   when the locked situation is solved.
  ...
  * \param dstart A CCW dart dstart
  * \param dend A CCW dart dend
  * \param elist A list containing all the edges crossing the spesified constraint
  *
  * \using
  *   ttl::swappableEdge
  *   ttl::swapEdgeInList
  ...
  */
  template <class TraitsType, class DartType>
    ...
    bool erase; // indicates if an edge is swapped away from the constraint such that it can be moved to the back of the list
    bool locked = false;
    do {
      int noswap = 0;
      it = elist.begin();
      ...
      while(it != elist.end()) { // ??? change this test with counter > dartsInList
        ...
        if (ttl::swappableEdge<TraitsType, DartType>(*it)) {
          // Dyn & Goren & Rippa 's notation:
          // The node assosiated with dart *it is denoted u_m. u_m has edges crossing
          // the constraint named w_1, ... , w_r . The other node to the edge assosiated with dart *it is w_s. 
          ...
          // If there is a looked situation, but we do an ordinary swap, it should be treated as if we were 
          // not in a locked situation!!

          // The flag swap_away indicates if the edge is swapped away from the constraint such that
          // it does not cross the constraint.
          bool swap_away = (crossesConstraint<TraitsType>(dstart, dend, *it, tmp) && !crossesConstraint<TraitsType>(dstart, dend, op1, op2));
          if (swap_away || locked) {
...
}; // end ttl_constr namespace scope

namespace ttl { // (extension)
  
  /** @name Constrained Triangulation */
  //@{
  /** Insert an edge (constraint) between two existing nodes in a triangulation.
  ...
  *   the constraint. 
  *
  * \param dstart A CCW dart with the start node of the constraint as the source node
  * \param dend A CCW dart with the end node of the constraint as the source node
  * \param optimizeDelaunay If set to \c true, the resulting triangulation will be
  ...
  *
  * \retval A dart representing the constrained edge.
  *
  * \require
  * - \ref hed::TTLtraits::orient2d "TraitsType::orient2d(DartType&, DartType&, DartType&)"
  * - \ref hed::TTLtraits::swapEdge "TraitsType::swapEdge"
  *
  * \using
  * - ttl::optimizeDelaunay if \c bool \c optimizeDelaunay is set to \c true
  *
  * \par Assumes:
  ...
  */
  template <class TraitsType, class DartType>
    DartType insertConstraint(DartType& dstart, DartType& dend, bool optimizeDelaunay) {
    ...
    DartType next_start = ttl_constr::findCrossingEdges<TraitsType, DartType>(dstart, dend, elist);
    ...
    // (elist can also be empty in the case of invalid input data (the constraint is in a non-convex area)
...
#endif
      insertConstraint<TraitsType,DartType>(next_start, dend, optimizeDelaunay);
    }
